...
- **Phase 4 — Launch readiness.** Azure deployment (retires the tunnel, turns Blob on), CI/CD, an automated test suite, waitlist access, quotas + write idempotency, observability + alerting, data lifecycle (delete/export/consent/privacy), backups + DR, and a second security pass. Test suite, waitlist, quotas, and the post-quota live checks are done. This is the "safe to invite strangers" gate.
