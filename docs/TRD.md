...
- **Subscriptions:** Engram docs require an active subscription for chat (403 otherwise). `personas.subscribe` needs workspace-admin rights; the live key has returned 403 `org:manage` (that string is not in the public docs). Independently, chat/retrieve/converse have still worked without a subscription on this alpha. **App published list is who may see a persona. Subscribe on first talk** so a future 403 cannot surprise us. Isolation does **not** depend on subscribe succeeding: gateway only mints an Engram id from an authenticated member, and `TurnRunner` refuses a session whose stored ids do not match. Private pools stay scoped per `(persona, Engram user_id)`. See [ENGRAM.md](ENGRAM.md) §4.
