...
- **Engram is the brain and the memory.** Which Engram call answers a turn is now config (`BRAIN_MODE`), and both paths keep memory in Engram:
  ...
  - **`retrieve` (default).** Two scoped `retrieve` calls in parallel (shared + private, each with its own `top_k`); the answer model composes from two labelled lists (`persona_memories` and `caller_memories`) under the same fact lock, and `personas.converse` writes both sides of the turn back to the caller's private pool off the reply path. SDK 0.4.0 `personas.retrieve` is shared-only on backend 0.5.0; we post `scope` ourselves. Measured ~2.5s to first word, ~4s on a live spoken call (before the second retrieve; the extra call runs concurrently so added latency is the slower of the two, not the sum).
...
- **Reply path (`BRAIN_MODE=retrieve`, default):** two parallel `POST .../retrieve` calls with `scope` from config (shared and private). Read `text` and `tenant` off **each row**. `may_ground` runs on every row of both pools. The answer payload is `{persona_memories, caller_memories, history, question, voice_config}` (key names from config). `top_k` is per pool. Member turns never send `user_id`; an org-key fallback is shared-only.
