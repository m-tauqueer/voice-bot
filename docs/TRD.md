...
- **BYO-LLM public reachability in local dev** — Aura sittings still need `BYO_LLM_PUBLIC_URL` (a tunnel or public worker URL Deepgram can call). Fish sittings think through the gateway at `WORKER_URL`, so the tunnel is not required for that path; see [SHIPPED.md](SHIPPED.md).
