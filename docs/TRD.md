...
- **Subscriptions:** Engram docs require an active subscription for chat (403 otherwise) and call subscribe workspace-admin; they never name `org:manage`. The live API does: this worker key is `org_admin` but scoped to `memory:read`/`write` plus billing/members/metrics/tokens **read**, so `personas.subscribe` and `subscribers` 403 `org:manage`. Independently, chat/retrieve/converse have still worked without a subscription on this alpha. **Grant code is done** (first talk, per persona). Filling Engram’s Subscribers list still needs a key or dashboard session that can actually subscribe. **App published list is who may see a persona.** Isolation does **not** depend on subscribe succeeding: gateway only mints an Engram id from an authenticated member, and `TurnRunner` refuses a session whose stored ids do not match. Private pools stay scoped per `(persona, Engram user_id)`. See [ENGRAM.md](ENGRAM.md) §4.
...
- **Isolation:** a user can never read another user's conversation, and a member's Ada history is not their Nova history. Engram private pools stay scoped per `(persona, Engram user_id)`. The app never forges tenants and refuses a session whose stored user / persona / Engram ids do not match. Subscribe is not the isolation control. Mandatory.
