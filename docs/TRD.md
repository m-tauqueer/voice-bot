...
- **Subscriptions:** Engram docs require an active subscription for chat (403 otherwise). Live subscribe needs `org:manage`; `members.add` needs `members:manage`. First talk joins People by Google email, persists Engram’s `user_id`, then subscribes that persona, then talks as that id. Fail closed if join or subscribe fails (except already subscribed). App published list is who may see a persona. Isolation is still the private tenant `{org}:{persona}:{user}`. Gateway mints a placeholder `engram_user_id` at admit; the worker replaces it with Engram’s id on first join. See [ENGRAM.md](ENGRAM.md) §4.
