...
- **Split pipeline specifics:** STT streaming with `interim_results=true` and `endpointing`; reconstruct utterances from `is_final`/`speech_final`; Fish TTS over WebSocket (PCM) when the provider is Fish and a Fish id is present, otherwise Aura TTS over WSS if that unused fallback is ever turned on; barge-in via abort/Clear + local playback flush.
