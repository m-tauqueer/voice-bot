...
- **Engram-side per-member private memory is broken by our own credential model, not blocked externally.** Every persona call goes out on one org API key, so Engram resolves each member's conversation to the key owner and they share one private pool. Live-verified 8 Sep 2026 that a member session token from `auth.login` fixes it — evidence in [ENGRAM_MEMBER_PRIVATE_WORKAROUND.md](ENGRAM_MEMBER_PRIVATE_WORKAROUND.md), plan in [ENGRAM_PRIVATE_ROLLOUT.md](ENGRAM_PRIVATE_ROLLOUT.md), contract in [ENGRAM.md](ENGRAM.md) §2.3. The private-pool half of this manual test cannot pass until that rollout lands; the app-side half (session ownership, persona pin, published gate) passes and `npm run isolation` is green.
