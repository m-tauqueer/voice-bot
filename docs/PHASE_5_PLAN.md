...
Built. Leftover **live sittings** are in §4. Current product work is [CALLER_MEMORY_PLAN.md](CALLER_MEMORY_PLAN.md). Fish/UI is paused in [PHASE_6_PLAN.md](PHASE_6_PLAN.md). Owner: Tauqueer. Map: [README.md](README.md). Read [AGENTS.md](../AGENTS.md), [CONTEXT.md](CONTEXT.md), [PRD.md](PRD.md), [TRD.md](TRD.md), and [ENGRAM.md](ENGRAM.md) first. **Do only the part Tauqueer names.** Do not start [FUTURE.md](FUTURE.md) until named.
...
- **Engram-side per-member private memory works as of 8 Sep 2026.** It had been broken by our own credential model, not blocked externally: Every persona call goes out on one org API key, so Engram resolves each member's conversation to the key owner and they share one private pool. Live-verified 8 Sep 2026 that a member session token from `auth.login` fixes it — evidence in [ENGRAM_MEMBER_PRIVATE_WORKAROUND.md](ENGRAM_MEMBER_PRIVATE_WORKAROUND.md), plan in [ENGRAM_PRIVATE_ROLLOUT.md](ENGRAM_PRIVATE_ROLLOUT.md), contract in [ENGRAM.md](ENGRAM.md) §2.3. Both halves of this manual test now pass: per-subscriber isolation and per-member private writes were confirmed by hand, and `npm run isolation` asserts turn grounding as well as the memory panel.
...
1. **Blocked on Engram conversation subject** — native `chat`/`retrieve`/`converse` still use the API key’s private pool ([ENGRAM.md](ENGRAM.md) §2.3). **Workaround researched and planned in two phases** (stop leak first, then subject ingest + `user_memories`): [ENGRAM_PRIVATE_ROLLOUT.md](ENGRAM_PRIVATE_ROLLOUT.md). Evidence: [ENGRAM_MEMBER_PRIVATE_WORKAROUND.md](ENGRAM_MEMBER_PRIVATE_WORKAROUND.md). Do not implement until Tauqueer names a phase and a part from that rollout. Old admin-pool converse data stays disposable.
