...
**Private recall reads (9 Sep 2026).** Backend 0.5.0 made SDK 0.4.0 `retrieve` shared-only. The worker now posts two scoped retrieves in parallel (`scope` and per-pool `top_k` from config, never `user_id` on a member turn). The answerer receives `{persona_memories, caller_memories, …}` so a self-directed question is not answered from the persona bio. An empty caller list is not filled from shared. The dashboard memory panel asks for `scope=private` and stays empty for a degraded member. Org-key fallback is shared only. `may_ground` is unchanged. Live first-word delta from the second retrieve is not re-measured this sitting; the calls share the existing write-back pool so added latency is the slower of the two, not the sum. Live `/chat` and `/voice` sittings for this read path are still outstanding.
