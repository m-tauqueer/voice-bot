...
3. **A second Google account.** Isolation is proven against two app users with real cookies (`npm run isolation`); the OAuth sign-up flow for a second account in a browser is not. Recorded in the security review; the live click still belongs to Tauqueer.
4. **Engram API key permissions.** `personas.subscribe` returns 403 `missing permission(s): ['org:manage']` for every identity tried, so sign-in cannot mirror subscriptions locally. **Accepted:** Engram does not gate `chat`, `retrieve` or `converse` on a subscription, and access control is enforced on `sessions.user_id` plus identity match. The `subscriptions` table stays empty until that key gains `org:manage`. People UI copy states subscription is not an access gate.
